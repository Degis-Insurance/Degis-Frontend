        ...
        <div class="row align-items-center pb-4">
          <div class="col-xl-5 pt-4" style="padding: 0 4%">
            <div class="d-flex pb-5">
              <img
                src="img/function/flight-comp.png"
          ...
          <div class="col-xl-7 pt-4" style="padding: 0 4%">
            ...
            <div class="row d-flex justify-content-between pt-5">
              ...
              <div class="col-xl-6 pt-1" align="right">
                <base-button style="width: 70%" @click="NewPolicyEvent"
                  >CONFIRM</base-button
                >
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async NewPolicy(timestamp, flight_no) {
      ...
      const premium = window.WEB3.utils.toBN(
        String(info.data.detail["premium"])
      );
      const departure_timestamp = window.WEB3.utils.toBN(
        String(info.data.detail["timestamp"])
      );
      const landing_timestamp = window.WEB3.utils.toBN(String(0));
      const deadline = window.WEB3.utils.toBN(
        String(info.data.detail["deadline"])
      );
      const signature = info.data.detail["signature"];

      const now = new Date().getTime();
      if (departure_timestamp - parseInt(now / 1000) < 86400) {
        alert("The departure time is too close, please select another flight");
      }
